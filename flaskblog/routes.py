...
from flaskblog.models import User, Post, VideoCamera
...
@app.route("/post/check_camera", methods=['POST'])
@login_required
def check_camera():
...
def start_live_surveillance():
    ...
    #if post.author != current_user:
     #   abort(403)
    subprocess.call(" python livedetection.py ", shell=True)
    return redirect(url_for('home'))
...
def logs():
	form=GetDate()
	image_file = url_for('static', filename='profile_pics/' + current_user.image_file)
	return render_template('log.html', title='Logs',user=current_user,image_file=image_file,date_time=datetime.datetime.now(),form=form)
